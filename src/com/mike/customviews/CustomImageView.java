...
public class CustomImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		canvas.drawCircle(centerX, centerY, pressedRingRadius
				+ animationProgress, focusPaint);
	...
	private void hidePressedRing() {
		pressedRingWidth = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, DEFAULT_PRESSED_RING_WIDTH_DIP,
				getResources().getDisplayMetrics());
		obj_animator.setFloatValues(pressedRingWidth, 0f);
		obj_animator.start();
	}
	
//	public void showPressedAnimation(){
//		
//		obj_animator.setFloatValues(animationProgress, pressedRingWidth);
//		obj_animator.start();
//		
//	}
//	
...
//	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			final TypedArray a = context.obtainStyledAttributes(attrs,R.styleable.CircleButton2);
			color = a.getColor(R.styleable.CircleButton2_cb_color, color);
			pressedRingWidth = (int) a.getDimension(
					R.styleable.CircleButton2_cb_pressed_ring_width,pressedRingWidth);
			a.recycle();
		}
		...
		final int pressedAnimationTime = getResources().getInteger(
				ANIMATION_TIME_ID);
		obj_animator = ObjectAnimator
				.ofFloat(this, "animationProgress", 0f, 0f);
		obj_animator.setDuration(pressedAnimationTime);
	}
